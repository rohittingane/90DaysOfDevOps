...
Basic YAML syntax only, on your own computer
...
Full, real checking by the actual Kubernetes API server
...
Create or update a resource based on a YAML file
...
List Pods with extra detail (IP, Node)
...
List only Pods matching a specific label

kubectl get pods -l key1=val1,key2=val2

List only Pods matching BOTH labels (AND)

kubectl describe pod <name>

Show full detailed info and event history for one Pod

kubectl logs <name>

Show the printed output/log history of a Pod's container
...
Create a Pod instantly, without any YAML file (imperative)

kubectl get pod <name> -o yaml

Show the full, real YAML of a resource as it exists in the cluster

kubectl run <name> --image=<image> --dry-run=client -o yaml

Preview/generate a YAML template without creating anything

kubectl apply -f file.yaml --dry-run=client

Check basic YAML syntax only (no deep validation)

kubectl apply -f file.yaml --dry-run=server

Full validation by the real API server, without saving anything
...
Delete a Pod (or other resource) using its manifest file

Overall Takeaways From Today

Every Pod manifest needs 4 things: apiVersion, kind, metadata, spec.

containerPort in a manifest is just documentation — it does NOT expose a port to the outside world by itself.

The command field overrides what a container runs when it starts. This is essential for images like BusyBox that would otherwise exit immediately.

Declarative (write YAML + apply) is reusable and trackable — good for real projects. Imperative (kubectl run) is fast for quick testing but nothing is saved anywhere.

kubectl get pod <name> -o yaml shows the FULL real state (your input + Kubernetes' defaults + live status) — not just what you typed.

--dry-run=client -o yaml is a great shortcut for quickly generating a starting YAML template.

--dry-run=client only checks basic syntax. --dry-run=server does full, real validation. Always prefer --dry-run=server before applying anything that matters.

Labels let you group and filter resources, even completely unrelated ones — this is the foundation for how Services and Deployments will target Pods later.

Standalone Pods have NO self-healing. If deleted or lost, they are gone forever with nothing to recreate them. This is why Deployments exist for real production use .
